data modify storage cobblenests:storage Storage.Copy set from entity @s data.last_spawned_uuid

# the uuid is compared in /data storage to check if there 
scoreboard players set $not_uuid pokemon_respawn_cooldown 1
execute as @e[type=cobblemon:pokemon,distance=..64] run function cobblenests:spawns/check_spawned_entity_2
